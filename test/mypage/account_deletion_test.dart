...
class _FakePort implements AccountDeletionPort {
  _FakePort({this.requestResult, this.cancelResult, this.error});
...
void main() {
  group('SupabaseAccountDeletionRepository (RPC 계약)', () {
    test('요청: p_dry_run=false 명시 + p_cancelable_minutes 전달(기본값 의존 금지)',
        () async {
      ...
      expect(fn, 'account_deletion_request');
      expect(params['p_dry_run'], isFalse, reason: '서버 기본값(true) 의존 금지');
      expect(params['p_cancelable_minutes'], 30);
      expect(params['p_user_id'], 'u-1');
