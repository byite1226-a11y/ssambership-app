...
### [B6] 요금제 미확정 시 가격 표시
- **웹**: 활성 요금제 없으면 추천 기본가(55,000/114,900/249,900) 표시.
  근거(탐색): `lib/subscribe/mentorPlanPricing.ts`(확인필요 — 파일명/라인 미직접검증).
...
- **앱**: 3종(questionRoom·subscription·other). CR·환불·IQ는 other로 숨김(의도적). 다만 **공지·리뷰·멘토활동**도 other로 묻힘.
  근거: `lib/features/notifications/data/app_notification.dart:27-59`.
- **차이**: 단순화(일부 의도적) · **심각도: 중간** · **난이도: 보통**
- **확인필요**: 공지·리뷰·멘토활동 알림을 앱에서도 노출할지(현재 전부 숨김) 오너 정책.
...
- **맞춤의뢰(CR)**: 앱 범위 밖(흔적 없이 제외). 알림·통계·메뉴 미노출.
- **개별질문(IQ)**: 앱 범위 밖. **단, 오너 방침상 "앱에 넣을지 재검토" 대상** → 재검토 시 별도 설계 필요.
- **관리자 콘솔·회원가입 폼**: 앱 미제공(관리자 로그인은 앱에서 차단).
- **앱 내 결제/가격입력/충전 실행**: 전부 웹 브릿지(Commerce-Zero). 구독·충전·정산·결제관리는 웹 URL 열기.
- **커뮤니티 글·숏폼 작성**: 웹 전용(S9). 앱은 열람·댓글·반응·신고·내활동만. (→ §6의 작성 관련은 '누락'이 아니라 설계 제외)
